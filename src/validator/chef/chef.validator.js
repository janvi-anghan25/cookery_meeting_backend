...
import { sendVerificationEmail } from '../../repositories/chef/chef';
...
const isEmailRegistered = async (value) => {
  let emailExist = await chefModel.isExist({
    email: value,
  });
...
const emailValidation = async (value) => {
  let userExist = await chefModel.get({
    email: value,
  });
...
const tokenValidation = async (value) => {
  let userExist = await chefModel.isExist({
